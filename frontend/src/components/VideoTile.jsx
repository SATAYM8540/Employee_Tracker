...
//         socket.emit("webrtc-answer", { answer, adminSocketId: fromSocketId });
//       } catch (err) {
//         console.error("❌ Admin failed to handle offer:", err);
//       }
//     };

//     const onIce = async ({ candidate, userId }) => {
//       if (String(userId) !== String(emp._id)) return;
//       if (pcRef.current && candidate) {
//         try {
//           await pcRef.current.addIceCandidate(new RTCIceCandidate(candidate));
//         } catch (err) {
//           console.error("❌ Admin failed to add ICE", err);
//         }
//       }
//     };
...
//   return (
//     <div style={{ width: 320, border: "1px solid #ddd", borderRadius: 6, padding: 6, margin: 6 }}>
//       <div style={{ height: 200, background: "#000" }}>
...
//         <div style={{ fontSize: 12 }}>{emp.email}</div>
//         <div style={{ fontSize: 12 }}>
//           Status: <span style={{ color: status === "online" ? "green" : "gray" }}>{status}</span>
//         </div>
//         {status === "online" && <div style={{ fontSize: 12 }}>Duration: <b>{duration}</b></div>}
//       </div>
//     </div>
//   );
// }
...
export default function VideoTile({ emp }) {
  ...
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user?.id) socket.emit("register", { userId: user.id, role: "admin" });
  ...
  return (
    <div style={{ width: 320, border: "1px solid #ddd", borderRadius: 6, padding: 6, margin: 6 }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {status === "online" && (
          <div style={{ fontSize: 12 }}>Duration: <b>{duration}</b></div>
        )}
      </div>
    </div>
  );
