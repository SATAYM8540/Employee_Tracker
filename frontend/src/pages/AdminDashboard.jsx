...
//       // 🔹 Auto-start stream if employee comes online
//       if (status === "online") {
//         setActiveSessions((prev) => ({ ...prev, [userId]: true }));
//         socket.emit("request-stream", { employeeId: userId, adminSocketId: socket.id });
//       } else {
//         setActiveSessions((prev) => {
//           const c = { ...prev };
//           delete c[userId];
...
//       // 🔹 Auto-start streams for online employees
//       r.data.forEach((emp) => {
//         if (emp.status === "online") {
//           setActiveSessions((prev) => ({ ...prev, [emp._id]: true }));
//           socket.emit("request-stream", { employeeId: emp._id, adminSocketId: socket.id });
//         }
//       });
//     } catch (e) {
//       console.error(e);
...
//   const calculateDuration = (login, logout, status) => {
//     if (!login) return "-";
//     let endTime = status === "online" ? now : logout ? new Date(logout) : "-";
//     if (endTime === "-") return "-";
//     const diffMs = new Date(endTime) - new Date(login);
//     const minutes = Math.floor(diffMs / (1000 * 60));
//     const hours = Math.floor(minutes / 60);
...
//         const uid = log.user._id;
//         if (!grouped[uid]) {
//           grouped[uid] = { Name: log.user.name, Email: log.user.email, Role: log.user.role, TotalSeconds: 0, Sessions: [] };
//         }
//         const duration = log.durationSeconds || 0;
//         grouped[uid].TotalSeconds += duration;
//         grouped[uid].Sessions.push({
//           Login: new Date(log.loginTime).toLocaleString(),
//           Logout: log.logoutTime ? new Date(log.logoutTime).toLocaleString() : "-",
...
//       const data = [];
//       Object.values(grouped).forEach((emp) => {
//         data.push({ Name: emp.Name, Email: emp.Email, Role: emp.Role, "Total Time": `${Math.floor(emp.TotalSeconds / 3600)}h ${Math.floor((emp.TotalSeconds % 3600) / 60)}m` });
//         data.push({ Name: "---- Sessions ----" });
//         emp.Sessions.forEach((s) => data.push({ Login: s.Login, Logout: s.Logout, Duration: s.Duration }));
//         data.push({});
//       });
...
//     <div className="container">
//       <h2 style={{ color: "#b30000" }}>Admin Dashboard</h2>
//       <button onClick={handleLogout} style={{ background: "red", color: "white", padding: "8px 16px", border: "none", marginBottom: "20px", cursor: "pointer" }}>Logout</button>

//       <div style={{ marginBottom: "20px" }}>
//         <button onClick={() => downloadReport("week")} style={{ margin: "10px", padding: "8px 14px", background: "orange", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer" }}>⬇ Weekly Report (Excel)</button>
//         <button onClick={() => downloadReport("month")} style={{ margin: "10px", padding: "8px 14px", background: "purple", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer" }}>⬇ Monthly Report (Excel)</button>
//       </div>

//       <h3>Employees</h3>
//       <table border="1" cellPadding="8" style={{ borderCollapse: "collapse" }}>
//         <thead><tr style={{ background: "#f2f2f2" }}><th>Name</th><th>Email</th><th>Role</th><th>Login Date</th><th>Login Time</th><th>Logout Date</th><th>Logout Time</th><th>Duration</th><th>Status</th><th>History</th></tr></thead>
//         <tbody>
//           {employees.map((emp) => {
...
//                 <td>{logoutDate}</td>
//                 <td>{logoutTime}</td>
//                 <td>{calculateDuration(emp.latestLogin, emp.latestLogout, emp.status)}</td>
//                 <td style={{ color: emp.status === "online" ? "green" : "gray", fontWeight: "bold" }}>{emp.status}</td>
//                 <td><button onClick={() => viewHistory(emp)} style={{ background: "blue", color: "white", border: "none", padding: "4px 8px", cursor: "pointer", borderRadius: "4px" }}>View</button></td>
//               </tr>
//             );
//           })}
//         </tbody>
...
//           <div key={emp._id} style={{ textAlign: "center" }}>
//             <VideoTile emp={emp} />
//             {emp.status === "online" && !activeSessions[emp._id] && (
//               <button onClick={() => { setActiveSessions(prev => ({ ...prev, [emp._id]: true })); socket.emit("request-stream", { employeeId: emp._id, adminSocketId: socket.id }); }}
//                 style={{ marginTop: "6px", background: "green", color: "white", padding: "4px 8px", border: "none", cursor: "pointer", borderRadius: "4px" }}>Start Stream</button>
//             )}
//           </div>
//         ))}
//       </div>
...
//           <div style={{ background: "white", padding: 20, borderRadius: 8, width: "80%", maxHeight: "80%", overflowY: "auto" }}>
//             <h3>Attendance History - {selectedEmp.name} ({selectedEmp.email})</h3>
//             <table border="1" cellPadding="6" style={{ width: "100%", borderCollapse: "collapse" }}>
//               <thead><tr><th>Login Date</th><th>Login Time</th><th>Logout Date</th><th>Logout Time</th><th>Duration</th></tr></thead>
//               <tbody>{history.map(h => {
//                 const loginDate = new Date(h.loginTime).toLocaleDateString();
//                 const loginTime = new Date(h.loginTime).toLocaleTimeString();
//                 const logoutDate = h.logoutTime ? new Date(h.logoutTime).toLocaleDateString() : "-";
//                 const logoutTime = h.logoutTime ? new Date(h.logoutTime).toLocaleTimeString() : "-";
//                 const duration = h.durationSeconds ? `${Math.floor(h.durationSeconds / 3600)}h ${Math.floor((h.durationSeconds % 3600)/60)}m` : "-";
//                 return <tr key={h._id}><td>{loginDate}</td><td>{loginTime}</td><td>{logoutDate}</td><td>{logoutTime}</td><td>{duration}</td></tr>
//               })}</tbody>
//             </table>
//             <button onClick={() => setSelectedEmp(null)} style={{ marginTop: 10, background: "red", color: "white", padding: "6px 12px", border: "none", borderRadius: 6, cursor: "pointer" }}>Close</button>
//           </div>
//         </div>
//       )}
//     </div>
